...
async function redirectDependsOnRole(user: any, router: Router, profilesService: ProfileService, utilityService: UtilityService): Promise<void> {
  ...
  if (storedRoleId === roleId) {
    ...
    // Handle error messages for incorrect role logins
    if (roleId === 3) {
